import { E as sanitize_props, F as rest_props, x as push, G as fallback, y as setContext, V as copy_payload, W as assign_payload, J as bind_props, z as pop, S as attr_class, M as clsx, N as slot, I as spread_props, Q as escape_html, R as getContext, K as spread_attributes, Z as sanitize_slots } from './index-j4_QQaNE.js';
import { t as twMerge, a as twJoin } from './bundle-mjs-CaBOJI9t.js';
import { W as Wrapper } from './Input-BqhjcanO.js';
...
function GradientButton($$payload, $$props) {
  ...
  const $$restProps = rest_props($$sanitized_props, ["color", "shadow"]);
  push();
  ...
  divClass = twMerge($$sanitized_props.outline && "p-0.5", gradientClasses[color], shadow && "shadow-lg", shadow && coloredShadowClasses[color], group ? $$sanitized_props.pill && "first:rounded-s-full last:rounded-e-full" || "first:rounded-s-lg last:rounded-e-lg" : $$sanitized_props.pill && "rounded-full" || "rounded-lg", $$sanitized_props.class);
  if ($$sanitized_props.outline) {
    $$payload.out += "<!--[-->";
    $$payload.out += `<div${attr_class(clsx(divClass))}>`;
...
function Textarea($$payload, $$props) {
  ...
  let wrappedClass = fallback($$props["wrappedClass"], "block w-full text-sm border-0 px-0 bg-inherit dark:bg-inherit focus:outline-hidden focus:ring-0 disabled:cursor-not-allowed disabled:opacity-50");
  ...
  Wrapper($$payload, {
    ...
    children: ($$payload2) => {
      if ($$slots.header) {
        $$payload2.out += "<!--[-->";
        $$payload2.out += `<div${attr_class(clsx(headerCls(true)))}><!---->`;
      ...
      if ($$slots.footer) {
        $$payload2.out += "<!--[-->";
        $$payload2.out += `<div${attr_class(clsx(headerCls(false)))}><!---->`;
...
function SpeedDial($$payload, $$props) {
  ...
  push();
  let defaultClass = fallback($$props["defaultClass"], "fixed end-6 bottom-6");
  let popperDefaultClass = fallback($$props["popperDefaultClass"], "flex items-center mb-4 gap-2");
  let placement = fallback($$props["placement"], "top");
  let pill = fallback($$props["pill"], true);
  let tooltip = fallback($$props["tooltip"], "left");
  let trigger = fallback($$props["trigger"], "hover");
  let textOutside = fallback($$props["textOutside"], false);
  let id = fallback($$props["id"], generateId, true);
  let name = fallback($$props["name"], "Open actions menu");
  let gradient = fallback($$props["gradient"], false);
  let open = fallback($$props["open"], false);
  setContext("speed-dial", { pill, tooltip, textOutside });
  let divClass;
  let poperClass;
  divClass = twMerge(defaultClass, "group", $$sanitized_props.class);
  poperClass = twMerge(popperDefaultClass, ["top", "bottom"].includes(placement.split("-")[0]) && "flex-col");
  let $$settled = true;
  let $$inner_payload;
  function $$render_inner($$payload2) {
    $$payload2.out += `<div${attr_class(clsx(divClass))}><!---->`;
    slot($$payload2, $$props, "button", {}, () => {
      if (gradient) {
        $$payload2.out += "<!--[-->";
        GradientButton($$payload2, spread_props([
          {
            pill,
            name,
            "aria-controls": id,
          ...
          $$restProps,
          {
            ...
            children: ($$payload3) => {
              $$payload3.out += `<!---->`;
              slot($$payload3, $$props, "icon", {}, () => {
                $$payload3.out += `<svg aria-hidden="true" class="w-8 h-8 transition-transform group-hover:rotate-45" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>`;
              });
              $$payload3.out += `<!----> <span class="sr-only">${escape_html(name)}</span>`;
            },
            $$slots: { default: true }
          }
        ]));
...
function SpeedDialButton($$payload, $$props) {
  const $$sanitized_props = sanitize_props($$props);
  const $$restProps = rest_props($$sanitized_props, [
    "btnDefaultClass",
    "name",
  ...
  let btnDefaultClass = fallback($$props["btnDefaultClass"], "w-[52px] h-[52px] shadow-xs p-2!");
  let name = fallback($$props["name"], "");
  let tooltip = fallback($$props["tooltip"], () => context.tooltip, true);
  let pill = fallback($$props["pill"], () => context.pill, true);
  let textOutside = fallback($$props["textOutside"], () => context.textOutside, true);
  let textOutsideClass = fallback($$props["textOutsideClass"], "block absolute -start-14 top-1/2 mb-px text-sm font-medium -translate-y-1/2");
  let textDefaultClass = fallback($$props["textDefaultClass"], "block mb-px text-xs font-medium");
  let btnClass;
  btnClass = twMerge(btnDefaultClass, tooltip === "none" && "flex-col", textOutside && "relative", $$sanitized_props.class);
  Button($$payload, spread_props([
    { pill, outline: true, color: "light" },
    $$restProps,
    {
      class: btnClass,
      children: ($$payload2) => {
        ...
        if (tooltip !== "none") {
          $$payload2.out += "<!--[-->";
          $$payload2.out += `<span class="sr-only">${escape_html(name)}</span>`;
        } else if (textOutside) {
          $$payload2.out += "<!--[1-->";
          $$payload2.out += `<span${attr_class(clsx(textOutsideClass))}>${escape_html(name)}</span>`;
        } else {
          $$payload2.out += "<!--[!-->";
          $$payload2.out += `<span${attr_class(clsx(textDefaultClass))}>${escape_html(name)}</span>`;
        }
        $$payload2.out += `<!--]-->`;
      },
      $$slots: { default: true }
...
export { Drawer as D, SpeedDial as S, Textarea as T, SpeedDialButton as a };
//# sourceMappingURL=SpeedDialButton-DHYXaY6u.js.map
